...
export function useCharacterVoice(chatType) {
  ...
  const playCharacterVoice = async (messageId, text, characterData, userSettings, bypassCache = false) => {
    if (!messageId || !text || !characterData || !userSettings) { setPlayingAudioId(null); return; }
    try {
  ...
  return { playCharacterVoice, playingAudioId, voiceErrors };
}