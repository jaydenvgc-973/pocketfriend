...
import { detectAndSendWorldPhoneIntent } from "@/hooks/useWorldPhoneIntentSend";
...
export default function Chat({ chatTypeOverride } = {}) {
  ...
  const sendMessage = async (text, userImageUrl) => {
    ...
    // ── WORLD PHONE INTENT DETECTION + SEND ─────────────────────────────────
    // Handles: explicit names ("text Maya that..."), recipient-only ("text Devon"),
    // and pronouns ("text him now" → resolves "him" from conversation context).
    // Must run BEFORE LLM so we can gate character response on real write success.
    const { worldPhoneIntent, worldPhoneSendResult: _wpResult } = await detectAndSendWorldPhoneIntent({
      text,
      characterId,
      character,
    ...
    let worldPhoneSendResult = _wpResult;
...
• DO NOT drift into past topics, stored memories, or general summaries unless directly relevant to THIS request.`;
  ...
  "location_share_note": "Optional one-sentence note about why you're sharing or what you're doing there",\n  "scheduled_events": [\n    {\n      "description": "What will happen",\n      "trigger_time": "<ISO 8601 UTC datetime>"\n    }\n  ]\n}\nOnly include scheduled_events if a specific real-world action with a concrete time is committed to. Only include share_location:true when genuinely sharing location. Omit fields you don't use.\n\n${imageRule}`;
      ...
      // worldPhoneIntent being set means the pre-send path already fired (or was attempted).
      // worldPhoneSendResult?.data?.success===false means the send actually failed —
      // in that case we still run character-action detection to prevent false "I sent it" claims.
      const preSendActuallySucceeded = worldPhoneIntent && worldPhoneSendResult?.data?.success === true;
      if (!preSendActuallySucceeded && responseText) {
    ...
    dispatchPostSend({
      ...
      worldPhoneAlreadySent: !!(worldPhoneIntent && worldPhoneSendResult?.data?.success === true),
    });
