...
• DO NOT drift into past topics, stored memories, or general summaries unless directly relevant to THIS request.`;
  ...
  "location_share_note": "Optional one-sentence note about why you're sharing or what you're doing there",\n  "scheduled_events": [\n    {\n      "description": "What will happen",\n      "trigger_time": "<ISO 8601 UTC datetime>"\n    }\n  ]\n}\nOnly include scheduled_events if a specific real-world action with a concrete time is committed to. Only include share_location:true when genuinely sharing location. Omit fields you don't use.\n\n${imageRule}`;
      ...
      // this turn AND it succeeded — the send is already done, no need to re-detect and
      // re-send based on the character's confirmation language in the response.
      ...
      // "it definitely sent", "I can see the message", "I'm looking at it now", etc.).
      ...
      // honest uncertainty.
      //
      // BYPASS: When worldPhoneIntent is set AND the send succeeded, the character
      // is allowed to confirm "I sent the message" without triggering a DB lookup.
      // The send was just executed moments ago — we know the record exists.
      // Guard still fires for unverified claims in subsequent turns (no active intent).
      const wpSendSucceededThisTurn = !!(worldPhoneIntent && worldPhoneSendResult?.data?.success);
      if (responseText && characterId && currentUser?.email && !wpSendSucceededThisTurn) {
        try {
