...
import { buildWorldPhoneRetrievalContext } from "@/lib/worldPhoneRetrievalContext"; import { buildGroupMessageRetrievalContext } from "@/lib/groupMessageRetrievalContext";
...
If a QR code is present but cannot be decoded: return exactly the word "QR_UNREADABLE".`,
      ...
      const optionalContextPromises = {
        ...
        groupRetrieval: buildGroupMessageRetrievalContext({ characterId, character, userMessage: text, allCharacters: queryClient.getQueryData(["characters", currentUser?.email]) || [], base44 }).catch(() => null),
      };
      ...
      try { worldPhoneRetrievalResult = await Promise.race([optionalContextPromises.worldPhoneRetrieval, new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 3000))]); } catch { worldPhoneRetrievalResult = null; }
      const worldPhoneRetrievalBlock = worldPhoneRetrievalResult?.block || '';
      if (worldPhoneRetrievalResult) console.log(`[Chat] WP_RETRIEVAL | contact=${worldPhoneRetrievalResult.contactName} | messages=${worldPhoneRetrievalResult.messageCount} | char=${character?.name}`);
      let groupRetrievalResult = null;
      try { groupRetrievalResult = await Promise.race([optionalContextPromises.groupRetrieval, new Promise((_, reject) => setTimeout(() => reject(new Error('timeout')), 3000))]); } catch { groupRetrievalResult = null; }
      const groupRetrievalBlock = groupRetrievalResult?.block || '';
      if (groupRetrievalResult) console.log(`[Chat] GROUP_RETRIEVAL | group=${groupRetrievalResult.groupTitle} | messages=${groupRetrievalResult.messageCount} | ambiguous=${!!groupRetrievalResult.ambiguous} | char=${character?.name}`);
