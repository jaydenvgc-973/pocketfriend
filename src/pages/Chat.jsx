...
import { reconcileWorldStateForResponse } from "@/lib/worldStateReconciliationEngine";
...
export default function Chat({ chatTypeOverride } = {}) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = base44.entities.Message.subscribe((event) => {
      ...
      // Fires when a WP message involving this character arrives while the page is open.
      // Evicts canonical prompt cache so the next send re-fetches WP-aware context.
      // Schema proof: channel==='world_phone' is the authoritative discriminator.
      if (event.type === "create" && shouldInvalidateForWorldPhone(event.data, characterId)) {
        invalidateCanonicalCache(characterId, currentUser?.email, systemPromptCacheRef);
      }
...
If a QR code is present but cannot be decoded: return exactly the word "QR_UNREADABLE".`,
      ...
      if (worldPhoneRetrievalResult) {
        console.log(
          ...
          ` | is_group=${!!worldPhoneRetrievalResult.isGroup}` +
          ` | ambiguous=${!!worldPhoneRetrievalResult.ambiguous}` +
          ` | char=${character?.name}`
        );
      }
      ...
      try {
        const cachedLocs = await getLocations(async () => {
          const r = await base44.functions.invoke('fetchAllLocationsForUser', {});
          return r?.data?.locations || [];
        });
        if (Array.isArray(cachedLocs)) {
          cachedLocs.forEach(l => { if (l?.id) worldStateLocationMap[l.id] = l; });
        }
      } catch {
        worldStateLocationMap = {};
      }
      const lastUserMsgForElapsed = (() => {
        const userMsgs = recentMsgs.filter(m => m.sender_type === 'user' && m.id !== userMsg.id);
        return userMsgs[userMsgs.length - 1] || null;
      })();
      const worldStateReconciliation = reconcileWorldStateForResponse(
        character,
        allCachedCharsForWorldState,
        worldStateLocationMap,
        userSettings,
        new Date()
      );
...
Respond to ${worldName} naturally as you normally would.`;
      ...
      // CONFINEMENT ENVIRONMENT OVERRIDE: must be injected BEFORE imageRule
      // so the LLM reads the environment isolation mandate before it generates any image prompt.
      const confinementImageOverride = buildConfinementImageOverride(character);

      // JAIL/PRISON NARRATIVE CONTEXT: behavioral rules, movement schedule, tone, environmental
      // realism. Injected only when character is actively incarcerated (not house arrest).
      const jailConfinementContext = buildJailConfinementContext(character);
...
• DO NOT drift into past topics, stored memories, or general summaries unless directly relevant to THIS request.`;
      ...
      // ── CLOTHING AWARENESS CONTEXT ────────────────────────────────────────────
      // Injects visual clothing perception for co-present characters.
      // Non-blocking: empty string when no co-present characters have outfit data.
      // Proof: observer personality (traits) determines reaction; uniforms checked visually;
      // ordinary outfits never forced into memory; no duplicate clothing authority created.
      const clothingCoPresent = (coPresenceResult?.charactersPresentHere || [])
        .map(cp => (queryClient.getQueryData(["characters", currentUser?.email]) || []).find(c => c.id === cp.id))
        .filter(Boolean);
  ...
  "location_share_note": "Optional one-sentence note about why you're sharing or what you're doing there",\n  "scheduled_events": [\n    {\n      "description": "What will happen",\n      "trigger_time": "<ISO 8601 UTC datetime>"\n    }\n  ]\n}\n\nRESPONSE SIMPLICITY RULE — READ BEFORE CHOOSING YOUR FORMAT:\nThe simplest natural response is usually the best. Most turns need ONLY a single spoken line.\n\nDEFAULT (most common): Use "text_content" with one spoken reply. Omit "sequence" entirely.\n  Example: user says "Good morning" -> text_content: "Morning." — no sequence, no narrative, no extras.\n  Example: user says "How are you?" -> text_content: "Good, just waking up." — no sequence needed.\n\nOPTIONAL — use ONLY when the moment genuinely calls for it:\n• "sequence" array: Use ONLY when the character both speaks AND performs a meaningful physical action that adds to the moment. A single dialogue item in sequence is valid. Multiple items (dialogue + narrative + dialogue) are valid ONLY when the interaction naturally interleaves speech and action — not on every turn.\n• "narrative" items: Use ONLY when a physical action, gesture, or environmental event genuinely enriches the moment. Do NOT add narrative just because the schema supports it. "Morning." does not need a narrative about stretching or looking out a window.\n• "share_location": Use ONLY when the character is genuinely sharing their location.\n• "scheduled_events": Use ONLY when a concrete future action with a specific time is committed.\n\nFORMAT SEPARATION RULES:\n• Do NOT generate both "text_content" AND "sequence". Choose ONE. If you use "sequence", leave "text_content" empty.\n• NEVER put physical actions inside text_content or a dialogue item. Physical actions go ONLY in narrative items.\n• NEVER put spoken words inside a narrative item. Spoken words go ONLY in dialogue items.\n• Do NOT use asterisks, italics, parentheses, or any formatting to disguise actions as dialogue.\n\nThe existence of a field in the schema does NOT mean you must populate it every turn. Omit any field you don't use. A simple dialogue-only response is the most common and natural outcome.\n\n${imageRule}`;
      ...
      // On success: also strips any fabricated summary of what Character B supposedly said back
      //   (Character B's REAL response is generated by sendWorldPhoneMessage backend).
      // On failure: strips the false claim entirely from the response.
      // Skip handleCharacterWorldPhoneAction when the user explicitly triggered a send
      // this turn — the send is already done (or failed), the finalizer below handles
      // response cleanup based on the actual result.
      if (!worldPhoneIntent && responseText) {
        const actionResult = await handleCharacterWorldPhoneAction({
          responseText,
          character,
          characterId,
          conversationId: conversationIdRef.current || conversationId,
      ...
      // ── WORLD PHONE RESPONSE FINALIZER ───────────────────────────────────────
      // RESULT-DRIVEN: When the user explicitly requested a send this turn,
      // finalize the response based on the ACTUAL send result — not regex detection.
      //
      // Case 1 (send succeeded): Allow send confirmations ("I sent it"), strip
      //   delivery/visibility claims ("it definitely sent", "they have it now").
      // Case 2 (send failed): Strip ALL send + delivery claims, replace with
      //   honest uncertainty.
      // Case 3 (no intent this turn): Pass to the DB-verification guard below.
      if (responseText && worldPhoneIntent) {
        const wpActualResult = worldPhoneSendResult?.data || null;
        const finalizedResult = finalizeWorldPhoneResponse(responseText, wpActualResult, true);
        if (finalizedResult.finalized) {
          responseText = finalizedResult.text;
          console.log(
            `[Chat] WP_FINALIZER | case=${finalizedResult.case}` +
            ` | stripped=${finalizedResult.stripped.join(',')}` +
            ` | msg=${finalizedResult.message_id || 'none'}` +
            ` | char=${character?.name}`
          );
        }
      }
      ...
      // DB-VERIFICATION PATH: Only runs when no explicit intent was detected this turn.
      // Catches the class of responses where the character makes a World Phone state
      // claim in a follow-up turn ("is the message there?" → "yes I can see it")
      // without any new send having happened.
      if (responseText && characterId && currentUser?.email && !worldPhoneIntent) {
        try {
