...
export default function GroupChat() {
  ...
  const sendMessage = async (text) => {
    ...
    for (const char of selectedCharacters) {
      ...
      const recentMsgs = newMessages.slice(-30);
      const chatHistory = recentMsgs.map(m => {
        if (m.sender_type === "user") return `User: ${m.content}`;
      ...
      const response = await base44.integrations.Core.InvokeLLM({
        prompt: `${systemPrompt}\n\nYou are in a group conversation with ${otherNames} and the user. Respond naturally. You may agree or disagree with others. Keep it short and real.\n\nConversation:\n${chatHistory}\n\nRespond as ${char.name} only. Do not include "EMOTIONAL_STATE:" in this response.`,
      });
            ...
            {characters.map(c => (
              <div
                ...
                className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-colors ${
                  selectedIds.includes(c.id) ? "border-primary bg-primary/5" : "border-border bg-card"
                }`}
              >
  ...
  return (
    <div className="h-screen flex flex-col bg-background max-w-lg mx-auto">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-background/80 backdrop-blur-xl">
        <button onClick={() => setIsSetup(false)} className="text-muted-foreground hover:text-foreground">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <div className="flex -space-x-2">
          {selectedCharacters.slice(0, 3).map(c => (
            <CharacterAvatar key={c.id} character={c} size="sm" />
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto py-4 space-y-1">
        ...
        <AnimatePresence>
          {messages.map(msg => (
            <MessageBubble key={msg.id} message={msg} showName={true} />
          ))}
        </AnimatePresence>
        <AnimatePresence>
