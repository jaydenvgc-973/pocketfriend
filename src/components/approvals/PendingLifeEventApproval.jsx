...
export default function PendingLifeEventApproval({ characterId, character }) {
  ...
  const { data: pendingEvents = [] } = useQuery({
    ...
    enabled: !!characterId,
    // Reduced from 15s → 60s to prevent constant polling storms across all open chat sessions.
    // PendingLifeEvents are generated by backend automations, not real-time user actions,
    // so a 60s poll cadence is accurate enough without hammering the API.
    refetchInterval: 60000,
    // Don't refetch on window focus — this was a hidden source of duplicate calls on tab switching.
