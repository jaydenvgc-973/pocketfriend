...
export default function MessageBubble({ message, isUser, onReact, onDelete, onDeleteImage, onForward, onPlayVoice, isPlayingVoice, voiceError, time, onReloadImage }) {
  ...
  return (
    ...
    >
      {/* Narrative messages: plain italic text, no bubble */}
      {isNarrative ? (
        <p className="text-sm leading-relaxed whitespace-pre-wrap italic text-muted-foreground">{filterDashes(message.content)}</p>
      ) : (
        <>
          {/* Main bubble for character & user messages */}
            ...
            <div
              className={cn(
                ...
                isUser
                  ...
                  : "bg-secondary border border-border shadow-sm text-foreground"
              )}
            >
