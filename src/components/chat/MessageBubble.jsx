...
export default function MessageBubble({ message, showName = false, onReact, onDelete, onDeleteImage, hasVoice, onPlayVoice, isPlayingVoice }) {
  ...
  return (
    ...
    >
      ...
      <div className={`${isNarrative ? "max-w-2xl" : "max-w-[80%]"} flex flex-col ${isNarrative ? "items-center" : isUser ? "items-end" : "items-start"}`}>
        ...
        <AnimatePresence>
          {showDelete && (
            <motion.button
              ...
              className={`absolute z-50 ${isUser ? "-right-8" : "-left-8"} top-1/2 -translate-y-1/2 flex items-center justify-center p-1.5 rounded-full bg-destructive text-destructive-foreground hover:bg-destructive/90 transition-colors shadow-md`}
              onClick={(e) => {
                e.stopPropagation();
                onDelete(message.id);
                setShowDelete(false);
        ...
        {/* Action buttons positioned to the right of bubble */}
        {!isNarrative && !isUser && (
