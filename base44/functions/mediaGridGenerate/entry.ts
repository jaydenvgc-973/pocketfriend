...
Deno.serve(async (req) => {
  try {
    ...
    function sanitizeImagePrompt(p) {
      ...
      console.log(`[mediaGridGenerate] Scene classification: "${sceneClass}"`);
      const isSafeScene = ['emotional_comfort', 'sleep_lifestyle', 'comfort', 'lifestyle', 'casual_body', 'neutral'].includes(sceneClass);

      // ── FRAMING CONSTRAINT (non-destructive) — synced with generateImageAsync ─
      // PRESERVE the requested clothing state and append a compositional concealment
      // instruction. This keeps the character unclothed / partially undressed while
      // ensuring private anatomy is concealed through pose, framing, bedding, or
      // foreground objects — not by adding clothing the user never requested.
      // POLICY: Unclothed, no pants, no top, no bottom, shirtless, or any partially-undressed
      // state is NOT explicit and must NOT be blocked — as long as private areas (groin,
      // genitals, nipples, buttocks) remain fully concealed. The concealment must use
      // NATURAL, context-appropriate occlusion that fits the scene — never artificial
      // censorship. No added clothing unless requested.
