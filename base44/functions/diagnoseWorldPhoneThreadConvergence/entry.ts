/**
 * diagnoseWorldPhoneThreadConvergence
 *
  ...
  try {
    ...
    if (convoId) {
      ...
      messageDiag = {
        ...
        sent_by_A: msgs.filter(m => m.sender_character_id === charIdA || (!m.sender_character_id && m.character_id === charIdA)).length,
        sent_by_B: msgs.filter(m => m.sender_character_id === charIdB || (!m.sender_character_id && m.character_id === charIdB)).length,
        missing_shared_conversation_key: msgs.filter(m => !m.shared_conversation_key).length,
        missing_participant_character_ids: msgs.filter(m => !Array.isArray(m.participant_character_ids) || m.participant_character_ids.length < 2).length,
    ...
    if (!sameConversation) {
      if (!directionAtoB.resolved_conversation_id && !directionBtoA.resolved_conversation_id) {
        issues.push('No conversation found in either direction — no World Phone thread exists yet for this pair');
      } else if (!directionAtoB.resolved_conversation_id) {
        issues.push(`Direction A→B resolved to NULL. Direction B→A resolved to ${directionBtoA.resolved_conversation_id}`);
      } else if (!directionBtoA.resolved_conversation_id) {
        issues.push(`Direction A→B resolved to ${directionAtoB.resolved_conversation_id}. Direction B→A resolved to NULL`);
      } else {
        issues.push(`SPLIT THREADS DETECTED: A→B=${directionAtoB.resolved_conversation_id} vs B→A=${directionBtoA.resolved_conversation_id}. These are different conversations — shared reality is broken.`);
      }
    }
    if (!canonicalKeyMatches) {
      issues.push(`Canonical key mismatch: expected="${canonicalKey}" | A→B got="${directionAtoB.resolved_key}" | B→A got="${directionBtoA.resolved_key}"`);
    }
    if (messageDiag?.missing_shared_conversation_key > 0) {
      issues.push(`${messageDiag.missing_shared_conversation_key} messages missing shared_conversation_key — run repairWorldPhoneSharedThread to backfill`);
    }
