  ...
  try {
    ...
    for (const [userEmail, userChars] of Object.entries(byUser)) {
      ...
      for (const char of userChars) {
        ...
        // ── TIER 3.6: DECISION INTENT CONSUMPTION ────────────────────────────
        // The decision engine (simulateActiveCharacterNeeds) writes routing intents
        // like "heading home to sleep" / "heading home to freshen up" / 
        // "needs to find food". This tier reads those intents and routes the
        // character to the right destination. Without this, intents are created
        // but never consumed — the character stalls in "planning" forever.
        {
          const intentAction = char.decision_intent_action;
          const intentActivity = (char.current_activity || '').toLowerCase();
          const isRoutingIntent = intentAction && (
            intentActivity.includes('heading home') ||
            intentActivity.includes('returning home') ||
            intentActivity.includes('needs to find food') ||
            intentActivity.includes('getting hungry') ||
            intentActivity.includes('grabbing something to eat') ||
            intentActivity.includes('going home') ||
            intentActivity.includes('wash up')
          );
          ...
          if (isRoutingIntent) {
            ...
            // ── SLEEP / HYGIENE / REST routing → home ──────────────────────
            if (['sleep', 'hygiene', 'rest'].includes(intentAction)) {
              if (!atHome && homeId) {
                const homeLoc = userLocations.find(l => l.id === homeId);
                if (homeLoc && char.resolved_current_location_id !== homeLoc.id) {
                  const intentTravelRes = await base44.functions.invoke('createTravelSession', {
                    characterId:           char.id,
                    destinationLocationId: homeLoc.id,
                    travelReason:          `decision_intent_${intentAction}: routing home per decision engine`,
                    travelSource:          'autonomous_need',
                    ownerEmail:            char.owner_email,
                  }).catch(e => ({ data: { success: false, error: e.message } }));
                  const itd = intentTravelRes?.data || {};
                  if (itd.success) {
                    totalMoved++;
                    moveLog.push(`${char.name} → ${homeLoc.name} [DECISION_INTENT_${intentAction.toUpperCase()} → IN_TRANSIT ~${itd.duration_minutes}min] via decision engine`);
                    console.log(`[autonomousMovement] ✓ ${char.name}: DECISION INTENT routing → home (${intentAction}) for: "${char.current_activity}"`);
                    intentHandled = true;
                  } else if (itd.blocked) {
                    blockedLog.push(`${char.name}: decision intent ${intentAction} home routing blocked — ${itd.blocker_reason}`);
                    console.log(`[autonomousMovement] ${char.name}: DECISION INTENT ${intentAction} BLOCKED — ${itd.blocker_reason}`);
                  } else {
                    blockedLog.push(`${char.name}: decision intent ${intentAction} createTravelSession FAILED — ${itd.error}`);
                  }
                } else if (atHome) {
                  console.log(`[autonomousMovement] ${char.name}: DECISION INTENT ${intentAction} — already home, needs engine will execute`);
                  intentHandled = true;
                }
              }
            }

            // ── EAT routing → home or nearest food_drink ───────────────────
            if (intentAction === 'eat' && !intentHandled) {
              if (!atHome) {
                const foodLocs = userLocations.filter(l =>
                  l.category === 'food_drink' || l.category === 'grocery' ||
                  (l.category === 'home' && l.id === homeId)
                ).sort((a, b) => {
                  const aIsHome = a.id === homeId ? 1 : 0;
                  const bIsHome = b.id === homeId ? 1 : 0;
                  return aIsHome - bIsHome;
                });
                const bestFoodLoc = foodLocs[0];
                if (bestFoodLoc && char.resolved_current_location_id !== bestFoodLoc.id) {
                  const eatTravelRes = await base44.functions.invoke('createTravelSession', {
                    characterId:           char.id,
                    destinationLocationId: bestFoodLoc.id,
                    travelReason:          'decision_intent_eat: routing to food per decision engine',
                    travelSource:          'autonomous_need',
                    ownerEmail:            char.owner_email,
                  }).catch(e => ({ data: { success: false, error: e.message } }));
                  const etd = eatTravelRes?.data || {};
                  if (etd.success) {
                    totalMoved++;
                    moveLog.push(`${char.name} → ${bestFoodLoc.name} [DECISION_INTENT_EAT → IN_TRANSIT ~${etd.duration_minutes}min] via decision engine`);
                    console.log(`[autonomousMovement] ✓ ${char.name}: DECISION INTENT routing → ${bestFoodLoc.name} (eat) for: "${char.current_activity}"`);
                    intentHandled = true;
                  }
                } else if (bestFoodLoc && char.resolved_current_location_id === bestFoodLoc.id) {
                  console.log(`[autonomousMovement] ${char.name}: DECISION INTENT eat — already at food location`);
                  intentHandled = true;
                }
              } else {
                console.log(`[autonomousMovement] ${char.name}: DECISION INTENT eat — at home, needs engine will execute`);
                intentHandled = true;
              }
            }
