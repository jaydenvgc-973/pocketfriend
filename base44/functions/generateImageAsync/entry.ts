...
Photorealistic smartphone photograph. Ultra-detailed. Real human proportions. Not an illustration.${identityLock}`;
  ...
  const lnL=(locationName||'').toLowerCase();const zn=(zoneName||'').toLowerCase();const lcCat=(locCategory||'').toLowerCase();const _PUB=['social','food_drink','gym','religion','education','school','workplace','community','outdoor','business','medical','grocery','government','public'];const _PRIV=['home','hotel','shelter'];const isConfinement=lcCat==='jail_prison';const isPub=!isIso&&!isConfinement&&(_PUB.includes(lcCat)||(!_PRIV.includes(lcCat)&&!_PUB.includes(lcCat)&&(/\b(bar|nightclub|lounge|restaurant|diner|cafe|coffee shop|church|school|college|university|gym|fitness|park|stadium|arena|theater|cinema|venue|concert hall|mall|airport|shop|store|workplace|office building|hospital|clinic|library|museum|casino|hotel lobby|community center|sports bar|comedy club)\b/.test(lnL)||/\b(pool party|club|concert|beach party|festival|crowd)\b/i.test(pLow))));const isResid=!isIso&&!isConfinement&&(_PRIV.includes(lcCat)||(!_PUB.includes(lcCat)&&(/\b(home|apartment|bedroom|hotel room|shelter room|residential suite|private residence)\b/.test(lnL)||/\b(bedroom|living room|kitchen|bathroom|backyard|home office|residential)\b/.test(zn))));const isStaffZone=/\b(stockroom|stock room|back office|storage|break room|service area|staff area|kitchen)\b/.test(zn);const isCrowded=isPub&&/\b(packed|crowded|busy|swamped|lively|people everywhere|full house|standing room|sold out|noisy|loud|dance floor is full|line at the bar|shoulder to shoulder|wall to wall)\b/.test(pLow);const _jailSolitary=isConfinement&&/\b(solitary|isolation|iso|shu|special housing|the hole)\b/.test(zn);const _jailStaff=isConfinement&&!_jailSolitary&&/\b(staff|security|guard|control room|warden|administrative)\b/.test(zn);const _jailCell=isConfinement&&!_jailSolitary&&!_jailStaff&&/\b(cell|holding cell|single cell|two.?man cell)\b/.test(zn);const _jailDorm=isConfinement&&!_jailSolitary&&!_jailStaff&&/\b(dorm|dormitory|shared housing|open bay|pod)\b/.test(zn);const _jailCommon=isConfinement&&!_jailSolitary&&!_jailStaff&&/\b(day room|common room|commons|dayroom|rec room)\b/.test(zn);const _jailYard=isConfinement&&!_jailSolitary&&!_jailStaff&&/\b(yard|recreation yard|exercise yard|gym|weight room|outdoor rec)\b/.test(zn);const _jailVisit=isConfinement&&!_jailSolitary&&!_jailStaff&&/\b(visitation|visiting room|visitor|transport|intake|processing|medical|chapel)\b/.test(zn);const confinementRule=_jailSolitary?'SOLITARY CONFINEMENT: ⛔ ZERO background people. Subject is completely alone. No silhouettes, no other bodies.':_jailStaff?'STAFF/SECURITY ZONE: Guards and officers only in background. ⛔ No inmates unless escorted. No public/patrons language.':_jailCell?'INDIVIDUAL CELL: Subject alone unless zone/prompt explicitly states shared cell. No invented cellmate.':_jailDorm?'DORMITORY: Background inmates may appear at low controlled density — in bunks, seated. Institutional calm. Not a crowd.':_jailCommon?'DAY ROOM: Background inmates at moderate institutional activity — seated, watching TV, slow movement. Blurred, subordinate. Not party-packed.':_jailYard?'YARD/RECREATION: Background inmates may appear at moderate density — exercising, standing. Institutional context only. No social-party energy.':_jailVisit?'VISITATION/INTAKE: Guards, visitors, or other inmates may appear if contextually supported. Controlled institutional setting.':'CONFINEMENT (unspecified zone): Sparse institutional background. Background inmates only at low density if shared space is implied.';
  const ec=isIso&&expectedHumanCount===0?0:expectedHumanCount;
  const humanPurityBlock=`\n\n════════════════════════════════════════════════════════════\n⛔⛔⛔ HUMAN PRESENCE PURITY LAW — ABSOLUTE OVERRIDE ⛔⛔⛔\n════════════════════════════════════════════════════════════\n\nEXPECTED FOREGROUND SUBJECTS: ${ec}\n${ec===0?'→ ZERO declared foreground subjects.':ec===1?'→ EXACTLY ONE declared foreground subject. No undeclared foreground people.':'→ EXACTLY TWO declared foreground subjects. No undeclared foreground people.'}\n\nFOREGROUND PURITY (applies always):\n⛔ No undeclared people in the foreground competing with the declared subject(s)\n⛔ No partial people — arms, legs, hands of undeclared persons in the foreground\n⛔ No POV photographer body parts (over-the-shoulder, hands in frame)\n⛔ No foreground reflections of undeclared people in mirrors or glass\n⛔ Location owners/workers/residents may NOT appear as foreground subjects unless explicitly named\n\n${isIso?'ISOLATION ACTIVE: zero humans total anywhere. No hands, silhouettes, or reflections.\n\n':''}BACKGROUND OCCUPANCY RULE FOR THIS SPECIFIC SCENE:\n${isStaffZone?'STAFF-ONLY ZONE: Only appropriate staff/employees may appear in the background. ⛔ NO customers, patrons, or members of the public behind the employee boundary.':isConfinement?confinementRule:isResid?'PRIVATE RESIDENTIAL: ⛔ DO NOT invent strangers, neighbors, visitors, or filler people. Occupancy = actual known presence only. If no other person is contextually established, the character is alone.':isPub?(isCrowded?'PUBLIC SOCIAL — ACTIVE CROWD: The prompt describes a busy/packed/crowded environment. ✅ Background crowd IS part of the scene reality — include blurred, non-specific, visually subordinate background figures. Subject is foreground focus. ⛔ DO NOT erase the crowd because the subject is the focus. Subject focus ≠ empty background.':'PUBLIC SOCIAL: Context-appropriate background figures are permitted as out-of-focus, non-specific environmental texture. Match the natural activity level of this location and zone. ⛔ Do not make an active venue appear sterile and empty.'):'CONTEXT-APPROPRIATE: Match the environment. Active public spaces may have background figures (blurred, subordinate). Private/quiet spaces: minimal or none.'}\n\nGENERATION INVALID IF:\n🚫 Undeclared person appears in the foreground competing with declared subject(s)\n🚫 Background contradicts the occupancy rule above (e.g. empty when described as packed)\n🚫 Any location-associated person appears as a foreground subject without being named\n════════════════════════════════════════════════════════════`;
