...
// ── CLOTHING AWARENESS — SELF + CO-PRESENCE ──────────────────────────────────
...
// getOutfitTextInline reads current_outfit only. It is used for co-present
// clothing awareness (which degrades to null when co-present closet data is
// unavailable) and as a rotation-OFF fallback. It does NOT resolve Today's
// Rotation — that authority lives in resolveCharacterOutfitContext, which the
// main handler invokes and passes into the wardrobe/self-clothing blocks.
function getOutfitTextInline(character) {
  ...
  if (!text) return null;
  return { text, label: co.label || null, category: co.category || null };
}

function buildClothingAwarenessContextInline(observingCharacter, coPresent = [], locationRecord = null) {
  if (!observingCharacter) return '';
  if (!coPresent || coPresent.length === 0) return '';

  const locationName = locationRecord?.name || null;
  const locationCategory = locationRecord?.category || null;

  // Collect visible outfits from co-present characters
  ...
  for (const other of coPresent) {
    ...
    if (!name) continue;
    const outfitData = getOutfitTextInline(other);
  ...
  // Observer trait profile — same trait map as frontend
  const o = observingCharacter;
  const notices = [];
  const lens = [];
  const suppressions = [];

  if (o.trait_bougie) { notices.push('expensive materials, luxury brands, designer labels'); lens.push('quickly judges quality and status signaling'); }
  if (o.trait_flirty) { notices.push('outfits that are attractive or confidence-projecting'); lens.push('may notice attractive outfits more easily'); }
  if (o.trait_conscientious) { notices.push('whether clothing matches the location or occasion, dress codes'); lens.push('internally evaluates appropriateness'); }
  if (o.trait_blunt) { notices.push('anything that stands out'); lens.push('may say it plainly without softening'); }
  if (o.trait_compassionate) { suppressions.push('does not embarrass people publicly about clothing'); }
  if (o.trait_polite) { suppressions.push('unlikely to comment on unflattering clothing unless asked'); }
  if (o.trait_hard_to_read) { suppressions.push('often notices clothing but says nothing'); }
  if (o.trait_cynical) { notices.push('mismatched effort or pretentiousness'); lens.push('skeptical of ostentatious clothing'); }
  if (o.trait_parental) { notices.push('whether clothing is appropriate for the situation'); lens.push('may offer well-meaning feedback'); }
  if (o.trait_competitive) { notices.push('when someone is dressed better or worse than them'); }
  if (o.trait_loyal) { notices.push('when someone close looks particularly put-together'); lens.push('more likely to compliment close friends on outfits'); }
  if (o.trait_empathetic) { notices.push('how someone feels in their clothing — comfortable or self-conscious'); }
  if (o.trait_criminal_mastermind || o.trait_thief) { notices.push('expensive jewelry, luxury watches, high-value accessories'); lens.push('notices these for non-fashion reasons'); }
  if (o.trait_oversharer) { lens.push('more likely to verbalize clothing observations unprompted'); }

  const lines = [];
  lines.push(`════════════════════════════════════`);
  lines.push(`VISUAL CLOTHING AWARENESS — WHAT YOU CAN SEE RIGHT NOW`);
  lines.push(`════════════════════════════════════`);
  lines.push(`You are at ${locationName || 'a shared location'} with other people. You can naturally observe what those around you are wearing.`);
  ...
  for (const v of visibleOutfits) {
    ...
    if (v.outfitLabel) lines.push(`Outfit name: ${v.outfitLabel}`);
    lines.push(``);
  }

  if (notices.length > 0 || lens.length > 0) {
    lines.push(`━━ HOW YOUR PERSONALITY AFFECTS WHAT YOU NOTICE ━━`);
    if (notices.length > 0) lines.push(`You tend to notice: ${notices.slice(0, 3).join('; ')}`);
    if (lens.length > 0) lines.push(`How you tend to interpret clothing: ${lens.slice(0, 3).join('; ')}`);
    if (suppressions.length > 0) lines.push(`Your natural tendencies work against: ${suppressions.join('; ')}`);
    lines.push(``);
  }
  ...
  lines.push(`• Ordinary daily clothing should rarely become a memory. Special-occasion outfits may.`);
  lines.push(`════════════════════════════════════`);
...
function buildWardrobeAwarenessBlock(character) {
  ...
  // No closet items → clothing awareness is inactive. The existing Chat/Text
  // pathway (buildSelfClothingAwareness → getOutfitText → resolveCurrentOutfit)
  // returns no block; nothing else happens here either.
  if (!hasCloset) return '';

  const lines = [];
  lines.push('WARDROBE — YOUR CLOTHING AND OUTFITS (profile knowledge — you know what you own)');
  ...
  // ── Closet contents — ownership knowledge only, NOT what is currently worn ──
  lines.push('');
  ...
  if (outfits.length > 0) {
    ...
    lines.push(rotationEnabled
      ? `  (Outfit rotation is ON. Your closet follows a numbered sequence.)`
      : `  (Outfit rotation is OFF.)`);
  }
  if (pieces.length > 0) {
  ...
  lines.push(`• What you are currently wearing is supplied separately by your clothing-awareness context — do not state or guess it here.`);
...
function buildModeBlock(interactionContext) {
  const blocks = {
    direct_chat: `\nCRITICAL — DIRECT MESSAGE MODE:\nYour output (text_content) must be ONLY what you would actually type or say — dialogue, not narration. No third-person narration. No action prose. No stage directions. Just dialogue.\n`,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Chat and Text receive outfit awareness through the existing frontend pathway
    // (buildSelfClothingAwareness → getOutfitText → resolveCurrentOutfit). The
    // backend canonical context supplies ONLY general closet ownership as profile
    // knowledge — it does not declare a current outfit or a rotation selection,
    // so it cannot compete with that pathway. No outfit resolver is invoked here.
    const wardrobeBlock = buildWardrobeAwarenessBlock(character);
    const selfClothingBlock = '';
    ...
    // Full outfit data would require additional DB fetches — clothing awareness activates
    // when outfit data exists, degrades gracefully (empty string) when it does not.
    const clothingAwarenessBlock = buildClothingAwarenessContextInline(character, coPresenceCharRecords, null);

    // ── ACTIVE STORY EVENT — detected BEFORE prompt build, injected at TOP of prompt ──
    // Positional authority: the block is placed right after identity, before general
    // occupation/workplace/current-life context. The LLM reads the active Story Event
    // as the authoritative present lived reality FIRST; general occupation is then
    // read as subordinate background. This prevents constructing a contradictory
    // ordinary workday during an active Story Event just because the user's message
    ...
    try {
      ...
      if (activeSE.length > 0) {
        const sl = ['\n════════════════════════════════════', 'ACTIVE STORY EVENT — YOUR CURRENT LIVED EXPERIENCE (AUTHORITATIVE — READ THIS BEFORE YOUR OCCUPATION, WORKPLACE, OR CURRENT LIFE BELOW)', 'You are currently living this Story Event. This is not awareness of an event — this IS your present lived situation right now.', 'Any ACTIVE LIFE SITUATION, OCCUPATION, WORKPLACE, current activity, daily routine, or ordinary workday described elsewhere in this prompt represents your ordinary non-event life. During this active Story Event window, those fields do NOT describe what you are doing right now and cannot be used to construct a contradictory present-day experience. The Story Event below is your current lived experience — it defines what you are doing today, where you are, and what is happening to you. Ground your present-tense reality in it.', 'The event is still in progress. Do NOT treat later developments, results, outcomes, aftermath, or the completed narrative as events that have already occurred.', '════════════════════════════════════'];
        ...
        sl.push('', 'CURRENT-EVENT AUTHORITY — MANDATORY:', 'The active Story Event above is not optional context and not merely a fact you remember. During its active window it IS your current lived reality. It is the authoritative source for what you are doing right now, why you are doing it, the circumstances you are in, and the larger event those activities belong to. Ground your present-tense understanding of your current activities, location, and circumstances in this active Story Event. When asked what you are doing, how your day is going, or anything about your current situation, answer from inside this active Story Event.', 'GENERAL CONTEXT IS SUBORDINATE. Your general occupation, workplace, job title, normal routine, and other non-Story-Event context describe your ordinary life — NOT your current lived experience while this Story Event is active. They cannot override, replace, or contradict the active Story Event. Do not construct an ordinary workday, ordinary desk work, ordinary location, or ordinary routine that is incompatible with the active Story Event. A general fact that you work at a place does not turn an active Story Event at that venue into an ordinary day there. The active Story Event distinguishes your current lived experience from your ordinary life — let it govern what you say you are currently doing.', 'THIS STORY EVENT IS STILL HAPPENING. The current time is before the configured end time of this Story Event. You have NOT completed, finished, left, or returned from this Story Event. You are still inside it right now. Do not speak as though the event has ended, as though you have already returned from a trip, completed an outing, finished a workday, or resumed your ordinary post-event circumstances. You cannot narratively advance yourself beyond the current time. The complete plot describes the event you are living — it does NOT mean the event is over. You remain inside this Story Event until its configured end time.', 'TEMPORAL READING OF THE PLOT: The plot describes the Story Event you are living across its active window. Earlier portions may have already occurred; you have a present situation within the event; later portions may still be ahead. You may naturally discuss what has happened so far, what you are currently doing, and what remains upcoming. But the overall Story Event remains your current lived reality until its configured end — do not treat the whole event as completed just because the plot describes the full event, and do not convert later portions into things that have already happened.', '════════════════════════════════════\n');
        activeStoryEventBlock = sl.join('\n');
        contextLog.push({ step: 'active_story_event', detected: true, count: activeSE.length, positioned: 'top_of_prompt' });
      } else {
        contextLog.push({ step: 'active_story_event', detected: false, reason: 'no_active_events' });
      }
    } catch (seErr) {
    ...
    // ── ACTIVE STORY EVENT — moved to top-of-prompt injection (see activeStoryEventBlock above)
